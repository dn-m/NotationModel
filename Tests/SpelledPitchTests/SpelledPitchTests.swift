...
class SpelledPitchTests: XCTestCase {
    ...
    func testOctaveBSharp() {
        let bsharp = SpelledPitch(Pitch.Spelling<EDO48>(.b, .sharp), 3)
    ...
    func testBQuarterSharp() {
        let bqsharp = SpelledPitch(Pitch.Spelling<EDO24>(.b, .quarter, .sharp), 3)
        XCTAssertEqual(bqsharp.pitch, 59.5)
    }

    func testBSharpDown() {
        let bsharpdown = SpelledPitch(Pitch.Spelling<EDO48>(.b, .sharp, .down), 3)
    ...
    func testComparableSameLetter() {
        ...
        let dsharp = SpelledPitch(Pitch.Spelling<EDO12>(.d, .sharp), 4)
        XCTAssert(d < dsharp)
        XCTAssert(dsharp > d)
    }
