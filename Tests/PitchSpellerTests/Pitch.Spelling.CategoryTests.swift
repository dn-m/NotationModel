//
//  Pitch.Spelling.CategoryTests.swift
//  PitchSpellerTests
//
...
class PitchSpellingCategoryTests: XCTestCase {
    ...
    func testLetterNamePredecessor() {
        let letterName: Pitch.Spelling.LetterName = .a
        let expected: Pitch.Spelling.LetterName = .g
        XCTAssertEqual(letterName.predecessor, expected)
    }

    func testLetterNameSuccessor() {
        let letterName: Pitch.Spelling.LetterName = .b
        let expected: Pitch.Spelling.LetterName = .c
        XCTAssertEqual(letterName.successor, expected)
    }

    func testNeutralLetterName() {
        XCTAssertEqual(Pitch.Spelling.LetterName.neutral(for: 0), .c)
        XCTAssertEqual(Pitch.Spelling.LetterName.neutral(for: 5), .f)
        XCTAssertEqual(Pitch.Spelling.LetterName.neutral(for: 9), .a)
        XCTAssertEqual(Pitch.Spelling.LetterName.neutral(for: 1), .c)
        XCTAssertEqual(Pitch.Spelling.LetterName.neutral(for: 10), .b)
        XCTAssertNil(Pitch.Spelling.LetterName.neutral(for: 8))
    }
